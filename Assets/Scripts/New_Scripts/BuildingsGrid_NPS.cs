...
public class BuildingsGrid_NPS : MonoBehaviour
{
    ...
    public void StartPlacingBuilding(Building buildingPrefab)
    {
        ...
        BuildingsGrid_KAPREM.npsBuild = true;
    ...
    private void Update()
    {
        if (!InputManager.isPaused)
        {
            ...
            if (flyingBuilding != null && (BuildingsGrid_KAPREM.pumpBuild || BuildingsGrid_KAPREM.ctiBuild || BuildingsGrid_KAPREM.ejBuild))
            {
                Destroy(flyingBuilding.gameObject);
                flyingBuilding = null;
                BuildingsGrid_KAPREM.npsBuild = false;
            ...
            if (flyingBuilding != null)
            {
                ...
                if (groundPlane.Raycast(ray, out float position))
                {
                    ...
                    if (x < 160 || x > GridSize.x - flyingBuilding.Size.x)
                    {
                        available = false;
                        //Debug.Log("Debug1: " + available + "; x = " + x + "; Grid = " + (GridSize.x - flyingBuilding.Size.x));
                    }
                    if (y < -9 || y > (GridSize.y - flyingBuilding.Size.y) * (-1))
                    {
    ...
    private void PlaceFlyingBuilding(int placeX, int placeY)
    {
        ...
        nps.SetActive(true);
        BuildingsGrid_KAPREM.npsBuild = false;
        BuildingsGrid_KAPREM.npsKey = true;
    }
}
