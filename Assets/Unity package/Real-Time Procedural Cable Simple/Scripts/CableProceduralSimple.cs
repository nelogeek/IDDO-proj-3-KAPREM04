...
public class CableProceduralSimple : MonoBehaviour
{
	...
	void Update () 
	{
        ...
        vectorFromStartToEnd = endPointTransform.position - startPointTransform.transform.position;
        // Setting the Start object to look at the end will be used for making the wind be perpendicular to the cable later.
        startPointTransform.transform.forward = vectorFromStartToEnd.normalized;
    ...
    void Draw()
	{
		...
		while(i < pointsInLineRenderer)
		{
			...
			Vector3 swayAtPoint = swayMultiplier * startPointTransform.transform.TransformDirection( new Vector3(Mathf.Sin(swayValue) * swayXMultiplier, Mathf.Cos(2 * swayValue + Mathf.PI) * .5f * swayYMultiplier, 0));
			// Calculate the waving due to wind for the current point i 

			// Calculate the postion with Sag.
			Vector3 currentPointsPosition = 
				startPointTransform.transform.position + 
				pointPosition + 
				(swayAtPoint + 
